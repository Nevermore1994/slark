...
TEST(Writer, open) {
    Writer writer("test.txt", "test open");
...
TEST(Reader, getPath) {
    Reader reader("test1.txt", ReaderSetting());
...
TEST(Reader, open) {
    ...
    auto writer = std::make_unique<Writer>("test_read.txt",  "testReader");
    ...
    Reader reader("test_read.txt", ReaderSetting([&str](DataPtr data, int64_t offset, IOState state){
        ASSERT_EQ(offset, 0);
...
TEST(File, isExist) {
    Writer writer1("test1.txt", "test file1");
    Writer writer2("test2.txt", "test file2");
    writer1.close();
    writer2.close();
