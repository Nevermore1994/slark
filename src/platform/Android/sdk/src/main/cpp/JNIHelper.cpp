...
std::string FromJVM::toString(JNIEnv* env, jstring str) noexcept {
    if (str == nullptr) {
        return {};
    }
...
DataPtr FromJVM::toData(JNIEnvPtr env, jbyteArray byteArray) noexcept {
    if (byteArray == nullptr) {
        return nullptr;
    }
    auto length = env->GetArrayLength(byteArray);
    auto dataPtr = std::make_unique<Data>(length);
    env->GetByteArrayRegion(byteArray, 0, length, reinterpret_cast<jbyte*>(dataPtr->rawData));
...
JNIReference<jintArray> ToJVM::toIntArray(JNIEnvPtr env, const std::vector<int32_t>& data) noexcept {
    ...
    env->SetIntArrayRegion(intArray, 0, size, data.data());
    return {env, intArray};
}

JNIReference<jbyteArray> ToJVM::toByteArray(JNIEnvPtr env, DataView data) noexcept {
    ...
    if (!data.empty()) {
        ...
        if (dataArray == nullptr) {
            ...
            env->SetByteArrayRegion(dataArray, 0, size, reinterpret_cast<const jbyte*>(data.view().data()));
        }
    } else {
...
JNIReference<jbyteArray> ToJVM::toNaluByteArray(JNIEnvPtr env, DataView data, bool isRawData) noexcept {
    auto size = static_cast<int32_t>(data.length());
    jbyteArray dataArray = nullptr;
    if (!isRawData) {
    ...
    if (!data.empty()) {
        ...
        if (dataArray == nullptr) {
            ...
            header.length = 4;
            env->SetByteArrayRegion(dataArray, 0, 4, reinterpret_cast<jbyte*>(header.rawData)); //nalu header length = 4
            env->SetByteArrayRegion(dataArray, 4, size, reinterpret_cast<const jbyte*>(data.view().data()));
        }
    } else {
        dataArray = env->NewByteArray(0);
    }
