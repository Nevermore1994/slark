...
std::string Native_AudioPlayer_createAudioPlayer(
    JNIEnv *env,
    jint sampleRate,
    jint channelCount
) {
    ...
    auto createMethod = JNICache::shareInstance().getStaticMethodId(
        audioPlayerClass,
        "createAudioPlayer",
        methodSignature
    );
    ...
    auto jPlayerId = reinterpret_cast<jstring>(env->CallStaticObjectMethod(
...
void Native_AudioPlayer_audioPlayerAction(
    JNIEnv *env,
...
uint64_t Native_AudioPlayer_getPlayedTime(
    ...
    auto method = JNICache::shareInstance().getStaticMethodId(
        audioPlayerClass,
        "getPlayedTime",
        signature
    );
...
std::string NativeAudioPlayer::create(
    uint64_t sampleRate,
    uint8_t channelCount
) {
    JNIEnvGuard envGuard(getJavaVM());
    auto playerId = Native_AudioPlayer_createAudioPlayer(
        envGuard.get(),
...
uint64_t NativeAudioPlayer::getPlayedTime(const std::string &playerId) {
    JNIEnvGuard envGuard(getJavaVM());
    return Native_AudioPlayer_getPlayedTime(
        envGuard.get(),
        playerId
    );
}

void NativeAudioPlayer::doAction(
    const std::string &playerId,
    AudioPlayerAction action
) {
    JNIEnvGuard envGuard(getJavaVM());
    Native_AudioPlayer_audioPlayerAction(
        envGuard.get(),
        playerId,
        action
    );
...
void NativeAudioPlayer::setConfig(
    const std::string &playerId,
    AudioPlayerConfig config,
    float value
) {
    JNIEnvGuard envGuard(getJavaVM());
    Native_AudioPlayer_setAudioConfig(
        envGuard.get(),
        playerId,
        config,
        value
