...
Java_com_slark_sdk_EGLRenderThread_createEGLContext(
    ...
    if (mainContext == nullptr) {
        LogE("failed to get main context for playerId: {}",
             playerId);
        return false;
    }
    ...
    if (error != EGL_SUCCESS) {
        LogE("failed to attach EGL context to surface, error: {}",
             error);
        return false;
    }
    ...
    SharedEGLContextManager::shareInstance().add(
        playerId,
        std::move(info));
    return true;
}
...
Java_com_slark_sdk_EGLRenderThread_destroyEGLContext(
    ...
    contextInfo->context
        ->releaseSurface(contextInfo->surface);
...
Java_com_slark_sdk_EGLRenderThread_swapGLBuffers(
    ...
    if (display == nullptr) {
        LogE("failed to get EGL display for playerId: {}",
             playerId);
        return false;
    }
    ...
    if (result == EGL_FALSE) {
        LogE("failed to swap buffers for playerId: {} {}",
             playerId,
             eglGetError());
    }
    auto error = eglGetError();
